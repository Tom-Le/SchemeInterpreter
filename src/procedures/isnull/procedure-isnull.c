...
static const char *_procedure_aliases[1] = {"not"};
static int _procedure_aliases_count = 1;
...
{
    return _procedure_aliases_count;
}

const char *scheme_procedure_get_alias(int index)
{
    if (index >= _procedure_aliases_count) return NULL;

    return _procedure_aliases[index];
}
